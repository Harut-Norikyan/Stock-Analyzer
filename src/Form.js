...
export default function Form() {
  ...
  const getTickers = (bool) => {
    if (isRequestInProcess) { return false; }
    isRequestInProcess = true;
    if (bool) { setIsShowNextButtonSpinner(true) }
    const form = {
      nextUrl,
    }
    Api.getTickers(form).then(response => {
  ...
  const analizeForm = () => {
    ...
    Api.analizeForm(form).then(response => {
        ...
        </div>
        {
          tickers && tickers.length ?
            tickers.map((tickerItem, index) => {
              ...
              >
                <div className={`card w-100 ${ticker === tickerItem.ticker ? "active-ticker" : ""}`}>
          ...
          <div className='row'>
            <div className='col-md-6'>
              <div className="form-group">
                <label htmlFor="percent" className='cursor-pointer mb-1'>Percent *</label>
                <input
                  id="percent"
                  type="number"
                  value={percent}
                  className={`form-control ${isInvalidSubmit && !percent ? "error-bordered" : ""}`}
                  aria-describedby="emailHelp"
                  placeholder="Percent"
                  onChange={(event) => onNumberChange(event, setPercent, 100)}
                />
                <small id="emailHelp" className="form-text text-muted">Lorem Ipsum is simply dummy text of the printing and typesetting industry.</small>
              </div>
            </div>
            <div className='col-md-6'>
              <div className="form-group">
                <label htmlFor="date" className='cursor-pointer mb-1'>Date *</label>
                <input
                  id="date"
                  type="date"
                  className={`form-control ${isInvalidSubmit && !startDate ? "error-bordered" : ""}`}
                  aria-describedby="emailHelp"
                  placeholder="Date"
                  value={startDate}
                  max={getToday()}
          ...
          <div className='d-flex justify-content-end mb-1'>
            {
              ticker ?
                <button
                  type="button"
                  className={
                    `btn btn-success position-relative save-button mx-1 
                      ${isShowSaveButtonSpinner || isShowNextButtonSpinner ? "disabled" : ""}
                    `
                  }
                  onClick={analizeForm}
                >
            ...
            <button
              type="button"
              className={
                `btn btn-primary position-relative  next-button mx-1 
                ${isShowSaveButtonSpinner || isShowNextButtonSpinner ? "disabled" : ""}
              `
              }
